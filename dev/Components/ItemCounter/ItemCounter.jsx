...
const ItemCounter = ({ id }) => {
  const { handlerCart, getQuantityById } = useContext(CartContext);

  const [unitQuantity, setUnitQuantity] = useState(getQuantityById(id));
  ...
  useEffect(() => {
    handlerCart(parseInt(id), unitQuantity);
  }, [unitQuantity]);
  ...
  return (
    <div className='ItemCounter'>
      ...
      <input type='text' value={unitQuantity} readOnly />
      <button
        onClick={() => {
          handlerCounter(true);
        }}
